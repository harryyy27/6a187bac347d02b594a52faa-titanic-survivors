...
def xgb_params(overrides: dict | None = None):
    """Hyperparameters for the XGBoost ensemble members, optionally overridden.

    ``overrides`` (if given) is merged on top of the defaults, letting
    callers (e.g. the container eval harness) tune individual tree/booster
    hyperparameters without duplicating the full default set.
    """
    params = {
        "max_depth": 4,
...
def train_xgb_ensemble(
    ...
    ``param_overrides`` is forwarded to :func:`xgb_params` so callers can
    tune booster hyperparameters (e.g. ``eta``, ``max_depth``) per run;
    ``early_stopping_rounds`` defaults to the project's original hardcoded
    value of 20 so existing callers (``run_pipeline``) are unaffected.
    """
