...
# Workflow: Health Check (Liveness/Readiness)

A user or monitoring system retrieves health status to verify service availability and basic readiness.
...
**Edge cases:**
  - If future readiness gate is false, handler returns 503 with structured error or downgraded status; update monitors accordingly
  - If system clock/tz misconfigured, ensure timestamps are emitted in UTC; formatter appends Z
  - If ORJSON fails on unexpected types (none here), handler must coerce to built-ins
**Test notes:** Unit test: GET /api/v1/health returns 200, JSON keys exist, content-type is application/json, Cache-Control is no-store, uptime_ms increases over time.
...
`/tmp/claude_task_plan_6a5930d00af5ba40433b276c_6a5bed2cf575302e212edd17_workflow_1.md`
...
Implement the full **Health Check (Liveness/Readiness)** as described above.
