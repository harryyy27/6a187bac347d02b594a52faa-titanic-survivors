...
# Workflow: Run API Locally (App Factory Boot + Lifespan)

Developer starts the FastAPI service using the app factory; settings load, logging initializes, middleware/routers/handlers attach, and readiness flips to true.
...
**Edge cases:**
  - Invalid .env value types cause pydantic validation error; service fails fast and logs error
  - No valid CORS origins in non-prod: log warning; continue with empty list, cross-origin browser calls will fail preflight
  - LOG_JSON=False in dev: logging falls back to console-friendly format but still harmonized
  - Starlette restriction: if CORS_ALLOW_ORIGINS='*', allow_credentials is forced False; warn if misconfigured
**Test notes:** Smoke test: process starts without unhandled exceptions; logs include ENV, API prefix, and CORS origins; /openapi.json reachable in non-prod.
...
`/tmp/claude_task_plan_6a5930d00af5ba40433b276c_6a5bed2cf575302e212edd17_workflow_0.md`
...
Implement the full **Run API Locally (App Factory Boot + Lifespan)** as described above.
